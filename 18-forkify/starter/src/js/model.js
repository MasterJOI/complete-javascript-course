...
const creatRecipeObject = function(data) {
  ...
  return {
    ...
    ...(recipe.key && { key: recipe.key }) //we add attribute only if it exists
  };
};
...
export const loadRecipe = async function(id) {
  try {
    ...
    // if recipe exist in state bookmarks, change bookmarked to true
    state.recipe.bookmarked = state.bookmarks
  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingredientArr.length !== 3) throw new Error('Wrong ingredient format!');
